...
function FeatureCard({ item, compact = false }: { item: typeof comparison[number]; compact?: boolean }) {
  ...
  return (
    <div className="h-full flex flex-col bg-gray-900/60 border border-gray-800 border-t-2 border-t-primary/60 rounded-2xl p-5 sm:p-6 backdrop-blur-sm hover:border-gray-700 hover:border-t-primary transition-colors">
...
function AnimatedFeatureCard({ item }: { item: typeof comparison[number] }) {
  const { ref, isVisible } = useFadeIn(0.2)

  return (
    <div
      ref={ref}
      className={`fade-in-section ${isVisible ? 'visible' : ''} h-full`}
    >
  ...
  return (
    <section className="section-padding bg-black relative overflow-hidden">
      ...
      <div className="container relative z-10">
        <div ref={ref} className={`fade-in-section ${isVisible ? 'visible' : ''} text-center max-w-3xl mx-auto mb-6 sm:mb-8 lg:mb-12 overflow-hidden`}>
          <h2 className="fade-child heading-2 mb-3 sm:mb-5 text-white">
            Why Luma costs less
          </h2>
          <div className="fade-child flex flex-wrap justify-center gap-x-1.5 sm:gap-x-2 text-sm sm:text-base lg:text-lg">
            {['No hidden fees.', 'No expensive hardware.', 'No long-term contracts.'].map((text) => (
              <span
                key={text}
        ...
        {/* Mobile compact grid */}
        <div className={`fade-in-section ${isVisible ? 'visible' : ''} grid grid-cols-2 gap-2 max-w-sm mx-auto lg:hidden`}>
          {comparison.map((item) => (
            <div
              key={item.feature}
        ...
        <div className="hidden lg:grid grid-cols-2 gap-4 md:gap-5 max-w-5xl mx-auto">
          {comparison.map((item) => (
            <AnimatedFeatureCard key={item.feature} item={item} />
          ))}
        </div>
      </div>
    </section>
